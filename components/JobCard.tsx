  ...
  return (
    ...
    >
      <div className="w-full md:w-64 h-48 md:h-auto shrink-0 relative">
        <img 
          src={job.images[0] || 'https://picsum.photos/seed/placeholder/400/300'} 
          alt={job.title}
      ...
      <div className="p-5 flex-grow flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-lg font-bold text-gray-900 line-clamp-2 leading-snug">{job.title}</h3>
          </div>
          <div className="flex items-center text-indigo-600 font-bold text-lg mb-2">
            <span className="text-sm mr-1">{job.salaryType === 'hourly' ? '時給' : job.salaryType === 'daily' ? '日給' : '月給'}</span>
          ...
          <div className="flex flex-wrap gap-1">
            {job.tags.slice(0, 4).map(tag => (
              <span key={tag} className="px-2 py-1 bg-gray-100 text-gray-600 text-[10px] rounded-full">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between text-xs text-gray-400">
          <span>更新日: {job.updatedAt.split('T')[0]}</span>
          <span className="text-indigo-600 font-medium">詳細を見る <i className="fas fa-chevron-right ml-1"></i></span>
        </div>
      </div>
    </div>
  );
