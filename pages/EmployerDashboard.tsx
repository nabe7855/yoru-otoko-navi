...
const EmployerDashboard: React.FC<EmployerDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    setJobs(allJobs.filter((j) => j.employerId === employer.id));

    const allApps = jobService.getApplications();
    // In a real app, we'd join with jobs to get only apps for this employer's jobs
    const employerJobIds = allJobs
      .filter((j) => j.employerId === employer.id)
      .map((j) => j.id);
    setApplications(allApps.filter((a) => employerJobIds.includes(a.jobId)));
  }, [employer.id]);
